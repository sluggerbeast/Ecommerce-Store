  ...
  return (
    <>
      <div className="pt-[5px] flex flex-col items-center">
        ...
        {cartList.length > 0 ? (
          <div className="bg-slate-50 w-[70vw] lg:w-[50vw] pt-3 ">
            <div className="pb-3">
              Total: <i className="fa fa-inr mr-1" />
              {totalCost}
            </div>
            <button
              onClick={handlePayClick}
              className="bg-black text-white p-3 w-[70vw] lg:w-[50vw]"
            >
              {!payPressed ? (
                <>
                  Pay <i className="fa fa-inr mr-1 ml-1" />
                  {totalCost}
                </>
              ) : (
                <>
          ...
          // if cart is empty during checkout redirect to home page.
          navigate("/") 
        )}
      </div>
...
function prePay() {
  ...
  const NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY =
    "pk_test_51OspyvSDjv5U4D3yA3W31pcuPIZALD0EWATfEvSp9pZ2vXB8RZT7iek8wLtmmZ3BDuym7CyjVl7i0HK3i4J6N9QA00gG8U9ELn";
  if (!stripePromise) {
...
function CheckoutBtn() {
  const SK =
    "sk_test_51OspyvSDjv5U4D3yQRFj07bOJ87TBKniGSMMRw5zUM8Fz8fAmsBIdEYkjghXEo8FrWseTIsZXp9Zn90yurvoYgA800tXIIhiFM";
  const NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY =
    "pk_test_51OspyvSDjv5U4D3yA3W31pcuPIZALD0EWATfEvSp9pZ2vXB8RZT7iek8wLtmmZ3BDuym7CyjVl7i0HK3i4J6N9QA00gG8U9ELn";
  ...
  useEffect(() => {
    async function handleCheckout() {
      ...
      const { error } = await stripe.redirectToCheckout({
        ...
        mode: "subscription",
        successUrl: `https://pareekstore.netlify.app/`,
